...
public class RetryPolicy implements RetryPolicyContext<HttpMethod> {
    ...
    public static RetryPolicy withSafeMethods(
        ...
        return new RetryPolicy(
                maxAttempts, backoff.toMillis(), maxBackoff.toMillis(), SAFE_METHODS);
    }
    ...
    public static RetryPolicy with(
            int maxAttempts, Duration backoff, HttpMethod... allowedMethods) {
        return new RetryPolicy(
                maxAttempts, backoff.toMillis(), MAX_BACKOFF_MS, Set.of(allowedMethods));
    }
